...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { invoice_id, amount, currency = "AED" }: CreateOrderRequest = await req.json();
    ...
    const usdAmount = currency === "AED" ? convertAEDtoUSD(amount) : amount;
      ...
      purchase_units: [
        {
          ...
          description: `Payment for Invoice #${invoice_id} (AED ${amount.toFixed(2)})`,
        },
      ],
      application_context: {
        return_url: `${req.headers.get("origin") || "http://localhost:5173"}/customer/payment-success`,
        cancel_url: `${req.headers.get("origin") || "http://localhost:5173"}/customer/payment-cancel`,
